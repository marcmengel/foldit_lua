...
-- version 1.4
-- * internal BUN fixing
-- * handling band.AddBetweenSegments returning 0
-- * misc cleanups
-- version 1.3 
-- * dialog for parameters
-- * cleanup on cancel
-- * better estimates due to finding boundary first
-- version 1.2
-- * initial estimate code
-- * optional WiggleAll(), so you can turn it off for endgame
-- * keep better scores if BUN score unchanged
...
function do_dialog()
    local ask = dialog.CreateDialog("UnBUN Tunables")
...
function do_internal_buns()
    --some BUNS are in our side...
    prev_bonus = filter.GetBonus(filt[satfilt])
    changed_any = false
    print("Part 1(2) checking for internal BUNS: "..boundary..".."..nsegs)
    for ups = boundary, nsegs do
        if (ups % 8 == 0) then
            pdone = math.floor((ups - boundary)* 100.0/nsegs)
            print( "["..pdone.."%] ")
        end
        if structure.IsHydrophobic(ups) then
            uaa = structure.GetAminoAcid(ups)
            structure.SetAminoAcid(ups, 'i')
            mod_bonus = filter.GetBonus(filt[satfilt])
            if mod_bonus > prev_bonus then
                -- our score gets better if we make this not polar,
                -- so keep it
                print("Making segment " .. ups .. " an 'i' gets rid of a BUN..")
                prev_bonus = mod_bonus
                recentbest.Save()
                save.Quicksave(99)
                changed_any = true
            else
    ...
    if changed_any then
        print("shaking...")
        -- if we changed any, then we may need (at least) a shake...
